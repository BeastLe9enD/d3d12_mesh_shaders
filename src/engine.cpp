...
namespace d3d12_mesh_shaders {
    void engine::create_window(uint32_t width, uint32_t height) noexcept {
        if(SDL_Init(SDL_INIT_EVERYTHING) != 0) {
            util::panic("SDL_Init");
        }

        _window = SDL_CreateWindow("d3d12_mesh_shaders", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, static_cast<int>(width), static_cast<int>(height), SDL_WINDOW_SHOWN);
        if(!_window) {
    ...
    engine::engine(bool debug_mode, uint32_t width, uint32_t height) noexcept {
        ...
        util::panic_if_failed(CreateDXGIFactory2(debug_mode ? DXGI_CREATE_FACTORY_DEBUG : 0, IID_PPV_ARGS(&_factory)), "CreateDXGIFactory2");
        if(debug_mode) {
